...
from iris.src.dataset import Batch
...
from iris.src.utils import compute_lambda_returns, LossWithIntermediateLosses
...
class ActorCritic(nn.Module):
    def __init__(self, act_vocab_size, use_original_obs: bool = False) -> None:
        ...
        self.conv1 = nn.Conv2d(3, 32, 3, stride=1, padding=1, device=self.device)
        self.maxp1 = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(32, 32, 3, stride=1, padding=1, device=self.device)
        self.maxp2 = nn.MaxPool2d(2, 2)
        self.conv3 = nn.Conv2d(32, 64, 3, stride=1, padding=1, device=self.device)
        self.maxp3 = nn.MaxPool2d(2, 2)
        self.conv4 = nn.Conv2d(64, 64, 3, stride=1, padding=1, device=self.device)
        self.maxp4 = nn.MaxPool2d(2, 2)

        self.lstm_dim = 512
        self.lstm = nn.LSTMCell(1024, self.lstm_dim, device=self.device)
        self.hx, self.cx = None, None

        self.critic_linear = nn.Linear(512, 1, device=self.device)
        self.actor_linear = nn.Linear(512, act_vocab_size, device=self.device)
