...
class PickleNode:
    def __init__(self, node: Node):
        ...
        for key, node in node.children.items():
            self.children[key] = PickleNode(node)
...
def batch_traverse(
    ...
    Returns:
        ...
        - virtual_to_play (:obj:`list`): The to_play list used in self_play collecting and trainin gin board games,
            `virtual` is to emphasize that actions are performed on an imaginary hidden state.
        - hidden_states: The model hidden states of the selected nodes.
    """
    parent_q = 0.0
...
def batch_backpropagate(
    ...
    for i in range(results.num):
        ...
        if to_play is None:
            ...
            results.nodes[i].expand(to_play=-1,
            ...
            results.nodes[i].expand(to_play=to_play[i],
                                    simulation_index=simulation_index,
                                    batch_index=i,
                                    reward=value_prefixs[i],
                                    policy_logits=policies[i],
        ...
        if to_play is None:
            ...
            search_paths = results.search_paths[i]
            stats_list = min_max_stats_lst.stats_lst[i]
            player_idx = to_play[i]
            value = values[i]
            backpropagate(
                search_paths, stats_list, player_idx, value, discount_factor
            )
