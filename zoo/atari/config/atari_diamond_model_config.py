...
def get_model_path_from_env_id(env_id: str):
    ...
    model_path = f"checkpoints/pretrained_diamond/atari_100k/models/{model_ckpt_name}.pt"
    return model_path
...
def get_configs(env_id: str):
    ...
    atari_muzero_config = dict(
        exp_name=f'data_diamond_mcts/{env_id[:-14]}_atari_ns{num_simulations}_upc{update_per_collect}-rr{replay_ratio}_seed0',
        env=dict(
            stop_value=int(1e6),
            env_id=env_id,
    ...
    atari_muzero_create_config = dict(
        ...
        policy=dict(
            type='diamond',
            import_names=['lzero.policy.diamond_policy'],
        ),
    )
