...
set EXPERIMENT_JOB_PARAMS "-hold_jid ${RUN_NAME}-Pull -N ${RUN_NAME}-Experiment -v RUN_NAME=$RUN_NAME,ENV_NAME=$ENV_NAME, GIT_BRANCH=$GIT_BRANCH,JOB_TYPE=\"run_playground\" $PROJECT_NAME/scripts/submit_experiment_job.sh"
